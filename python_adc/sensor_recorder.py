...
class MotionRecorder:
    ...
    def get_next_filename(self):
        ...
        return os.path.join(save_dir, f"sensor_{next_number:03d}.csv")
    ...
    def save_motion_data(self):
        ...
        if self.data_buffer:
            ...
            with open(filename, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["timestamp", "adc_x", "adc_y", "adc_z"])
    ...
    def plot_from_csv(self):
        ...
        with open(self.last_saved_file, "r") as f:
            ...
            next(reader)
            for row in reader:
                adc_x.append(float(row[1]))
                adc_y.append(float(row[2]))
                adc_z.append(float(row[3]))

        plt.figure(figsize=(10, 6))
        plt.plot(adc_x, "r-", label="X")
        plt.plot(adc_y, "g-", label="Y")
        plt.plot(adc_z, "b-", label="Z")
        plt.title("sensor value")
        plt.xlabel("numberOfDatas")
        plt.ylabel("difference_V")
        plt.legend()
        plt.grid()

        graph_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "sensor_graph"
        )
    ...
    def notification_handler(self, sender, data):
        """BLEからのデータ受信処理"""
        try:
            adc_x, adc_y, adc_z = struct.unpack("fff", data)
            timestamp = datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S.%f"
            )  # ミリ秒まで記録
            if self.is_recording:
                self.data_buffer.append([timestamp, adc_x, adc_y, adc_z])
